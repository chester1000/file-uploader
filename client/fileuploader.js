 ...
 * Submit your changes at: https://github.com/valums/file-uploader
 *
 * VERSION 2.0 beta
...
qq.addClass = function(element, name){
    if (!qq.hasClass(element, name)) element.className += ' ' + name;
};
...
qq.getByClass = function(element, className){
    ...
    for (var i = 0; i < len; i++){
        if (qq.hasClass(candidates[i], className)) result.push(candidates[i]);
    }
    return result;
};
...
qq.FileUploaderBasic.prototype = {
    ...
    _uploadFile: function(fileContainer){
        ...
        if (this._options.onSubmit(id, fileName) !== false){
            ...
            if (this._options.autoUpload) this._handler.upload(id, this._options.params);
            else {
                this._filesInProgress--;
...
qq.extend(qq.FileUploader.prototype, {
    ...
    _onProgress: function(id, fileName, loaded, total){
        ...
        if (loaded != total) text = percent + '% from ' + this._formatSize(total); // If still uploading, display percentage
        else text = this._formatSize(total); // If complete, just display final size
...
qq.UploadDropZone.prototype = {
    ...
    _attachEvents: function(){
        ...
        self._attach(self._element, 'dragover', function(e){
            ...
            if (effect == 'move' || effect == 'linkMove') e.dataTransfer.dropEffect = 'move'; // for FF (only move allowed)
            else e.dataTransfer.dropEffect = 'copy'; // for Chrome
...
qq.UploadButton.prototype = {
    ...
    _createInput: function(){
        ...
        if (window.attachEvent) input.setAttribute('tabIndex', "-1"); // it is IE or Opera
